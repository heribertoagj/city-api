...
public interface CityRepository extends JpaRepository<City, Long> {
    ...
    @Query(value = "SELECT ((SELECT point(latitude, longitude) FROM cidade WHERE id=?1) <@> (SELECT point(latitude, longitude) FROM cidade WHERE id=?2)) as distance", nativeQuery = true)
